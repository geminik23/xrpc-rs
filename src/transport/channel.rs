...
impl ChannelTransport {
    ...
    // Only for SharedMemoryTransport, because not all transports support non-blocking receive.
...
impl Transport for ChannelTransport {
    ...
    fn is_connected(&self) -> bool {
        // Always true. Channel is always connected.
        true
    }

    async fn close(&self) -> TransportResult<()> {
        // No-op. Channel is always connected.
        Ok(())
    }
...
mod tests {
    ...
    async fn test_transport_bidirectional() {
        ...
        t1.send(b"Hello from t1").await.unwrap();
        let msg = t2.recv().await.unwrap();
        assert_eq!(msg.as_ref(), b"Hello from t1");

        t2.send(b"Hello from t2").await.unwrap();
